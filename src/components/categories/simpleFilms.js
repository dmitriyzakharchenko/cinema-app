    ...
    if (films.length === 0) {
        return (
            <div className="category pt-3 pb-5">
                <Container>
                    <div className="text-white mb-3">Извините, этот фильм не был найден</div>
                    <Link to='/films' className="button">Вернуться назад</Link>
                </Container>
            </div>
        )
    }
                ...
                films.map((item) => {
                    return (
                        ...
                        <div className="category-nav">
                            <Link to='/films' className="button d-inline-block mt-2 mb-4">Назад</Link>
                            {/* <Link to={'/films' + '/' + 1} className="button d-inline-block mt-2 mb-4">Следующий фильм</Link> */}
                        </div>
